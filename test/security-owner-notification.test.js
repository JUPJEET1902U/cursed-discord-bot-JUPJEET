...
assert.equal(json.title, "Security alert • Unauthorized bot blocked")
assert.match(json.description, /blocked in \*\*Test Server\*\*/)
assert.equal(json.fields.find(field => field.name === "Bot response").value, "Banned")
assert.equal(json.fields.find(field => field.name === "Inviter response").value, "Quarantined")
assert.equal(json.footer.text, "CURSED • Server Protection")
